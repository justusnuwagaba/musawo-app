...
export default function ScheduleAppointmentScreen({ route, navigation }) {
  ...
  const handleConfirm = async () => {
    if (!selectedDate || !selectedTime) {
      showAlert('Missing details', 'Please pick a date and time.');
      return;
    }
    ...
    try {
      ...
      showAlert('Appointment scheduled', `Confirmed with ${patientName}.`, [{ text: 'OK', onPress: () => navigation.goBack() }]);
    } catch (err) {
      console.error('[ScheduleAppointmentScreen] error:', err);
      showAlert('Could not schedule', 'Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        <Text style={styles.doctorLine}>Scheduling for <Text style={styles.doctorName}>{patientName}</Text></Text>
        ...
        <Button title="Confirm schedule" onPress={handleConfirm} loading={submitting} style={styles.confirmButton} />
      </ScrollView>
    </SafeAreaView>
  );
}
